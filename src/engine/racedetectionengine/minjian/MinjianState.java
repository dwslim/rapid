...
public class MinjianState extends State{
	...
	public ArrayList<VectorClock> readVariableVCs;
	public ArrayList<VectorClock> writeVariableVCs;
	...
	public void initData(HashSet<Thread> tSet) {
		...
		initialize1DArrayOfVectorClocksWithBottom(this.threadVCs, this.numThreads, this.numThreads);
		...
		this.readVariableVCs = new ArrayList<VectorClock>();

		// initialize writeVariable
		this.writeVariableVCs = new ArrayList<VectorClock>();
	...
	private int checkAndAddVariable(Variable v){
		if(!variableToIndex.containsKey(v)){
			...
			this.numVariables ++;
			readVariableVCs	.add(new VectorClock(this.numThreads));
			writeVariableVCs	.add(new VectorClock(this.numThreads));
		}
